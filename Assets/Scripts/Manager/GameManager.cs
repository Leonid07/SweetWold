    ...
    public void StartGame(int levelValue)
    {
        if (levelValue <= 2)
        {
            for (int i = 0; i < enemies.Length; i++)
            {
                ...
                enemies[i].damage = (i * 10) + (enemies[i].damage * 1);
            }
        }
        else
        {
            int level = levelValue-1;
            ...
            for (int i = 0; i < enemies.Length; i++)
            {
                ...
                int rand = Random.Range(0, spriteEnemy.Length);
                enemies[i].spriteEnemy.sprite = spriteEnemy[rand];
                enemies[i].damage = (i * 10) + (enemies[i].damage * level);
            }
        }
    }
    public void LowLevelGame()
    {
        int countLevel = DataManager.InstanceData.mapNextLevel.indexLevel;
        countLevel--;
        for (int i = 0; i < enemies.Length; i++)
        {
            enemies[i].damage = enemies[i].standartDamage;
            int rand = Random.Range(0, spriteEnemy.Length);
            enemies[i].spriteEnemy.sprite = spriteEnemy[rand];
            enemies[i].damage = (i * 10) + (enemies[i].damage * countLevel);
        }
    }
}